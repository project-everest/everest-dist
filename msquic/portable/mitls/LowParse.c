...
uint32_t LowParse_Low_Int_write_u16(uint16_t x, LowParse_Slice_slice input, uint32_t pos)
{
  uint8_t bx = (uint8_t)x;
  uint32_t i_ = (uint32_t)1U;
  input.base[pos + i_] = bx;
  uint16_t x1 = x / (uint16_t)256U;
  uint8_t bx0 = (uint8_t)x1;
  uint32_t i_0 = (uint32_t)0U;
  input.base[pos + i_0] = bx0;
  uint32_t len = (uint32_t)2U;
  return pos + len;
}
