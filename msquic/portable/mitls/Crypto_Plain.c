...
static uint8_t FStar_List_Tot_Base_index__uint8_t(Prims_list__uint8_t *l, Prims_int i)
{
  if (i == (krml_checked_int_t)0)
    return FStar_List_Tot_Base_hd__uint8_t(l);
  else
    return
      FStar_List_Tot_Base_index__uint8_t(FStar_List_Tot_Base_tl__uint8_t(l),
...
Crypto_Symmetric_Bytes_store_bytes_aux(
  ...
  uint8_t *buf,
  uint32_t i,
  Prims_list__uint8_t *b
)
{
  if (i < len)
  {
    buf[i] = FStar_Seq_Base_index__uint8_t(b, FStar_UInt32_v(i));
    Crypto_Symmetric_Bytes_store_bytes_aux(len, buf, i + (uint32_t)1U, b);
  }
}
