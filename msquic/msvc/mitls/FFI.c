...
FFI_write_all_(
  Connection_connection c0,
  TLSInfo_id i10,
  FStar_Bytes_bytes buffer0,
  Prims_int sent0
)
{
  Connection_connection c = c0;
  TLSInfo_id i1 = i10;
  FStar_Bytes_bytes buffer = buffer0;
  Prims_int sent = sent0;
  while (true)
    ...
    else
    {
      ...
      if (scrut.tag == TLS_Written)
        sent = Prims_op_Addition(sent, size1);
      else
        return scrut;
    }
...
FFI_read_result FFI_read(Connection_connection c0)
{
  ...
  while (true)
  {
    ...
    if (!(scrut.tag == TLS_Complete))
      if (scrut.tag == TLS_Read && scrut.val.case_Read.tag == DataStream_Data)
      {
        ...
        return
          ((FFI_read_result){ .tag = FFI_Errno, .val = { .case_Errno = (krml_checked_int_t)0 } });
      ...
      else if (scrut.tag == TLS_ReadError)
      {
        ...
        FStar_Pervasives_Native_option__Parsers_Alert_alert
        description = scrut.val.case_ReadError.o;
        return
          (
            (FFI_read_result){
              .tag = FFI_Errno,
              .val = { .case_Errno = FFI_errno(description, txt) }
            }
          );
      }
        ...
        return KRML_EABORT(FFI_read_result, "unexpected FFI read result");
  }
  KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
    __FILE__,
    __LINE__,
  ...
  while (true)
    ...
    else if (l->tag == Prims_Cons)
    {
      ...
      else
      {
        ...
        return KRML_EABORT(TLSConstants_config, "");
      }
    }
    else
      return KRML_EABORT(TLSConstants_config, "unreachable (pattern matches are exhaustive in F*)");
  KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
    __FILE__,
    __LINE__,
    "unreachable, returns inserted above");
...
{
  Prims_string f = f0;
  Prims_list__K___Prims_string_Parsers_CipherSuite_cipherSuite *l = l0;
  while (true)
    if (l->tag == Prims_Nil)
      return
        (
    ...
    else if (l->tag == Prims_Cons)
    {
      ...
      return
        KRML_EABORT(FStar_Pervasives_Native_option__Parsers_CipherSuite_cipherSuite,
          "unreachable (pattern matches are exhaustive in F*)");
  KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
    __FILE__,
    __LINE__,
...
{
  Prims_string f = f0;
  Prims_list__K___Prims_string_Parsers_SignatureScheme_signatureScheme *l = l0;
  while (true)
    if (l->tag == Prims_Nil)
      return
        (
    ...
    else if (l->tag == Prims_Cons)
    {
      ...
      return
        KRML_EABORT(FStar_Pervasives_Native_option__Parsers_SignatureScheme_signatureScheme,
          "unreachable (pattern matches are exhaustive in F*)");
  KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
    __FILE__,
    __LINE__,
...
{
  Prims_string f = f0;
  Prims_list__K___Prims_string_Parsers_NamedGroup_namedGroup *l = l0;
  while (true)
    if (l->tag == Prims_Nil)
      return
        (
    ...
    else if (l->tag == Prims_Cons)
    {
      ...
      return
        KRML_EABORT(FStar_Pervasives_Native_option__Parsers_NamedGroup_namedGroup,
          "unreachable (pattern matches are exhaustive in F*)");
  KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
    __FILE__,
    __LINE__,
...
{
  Prims_string f = f0;
  Prims_list__K___Prims_string_EverCrypt_aead_alg *l = l0;
  while (true)
    if (l->tag == Prims_Nil)
      return
        (
    ...
    else if (l->tag == Prims_Cons)
    {
      ...
      return
        KRML_EABORT(FStar_Pervasives_Native_option__EverCrypt_aead_alg,
          "unreachable (pattern matches are exhaustive in F*)");
  KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
    __FILE__,
    __LINE__,
...
)
{
  ...
  *l = l0;
  ...
  while (true)
    ...
    {
      l =
        FStar_List_Tot_Base_tl__FStar_Pervasives_dtuple3_TLSInfo_logInfo_TLSInfo_pre_exportId_FStar_Bytes_bytes(l);
...
FFI_ext_filter(uint16_t ext_type0, Prims_list__Extensions_extension_ *e0)
{
  uint16_t ext_type = ext_type0;
  Prims_list__Extensions_extension_ *e = e0;
  while (true)
    if (e->tag == Prims_Nil)
      return
        ((FStar_Pervasives_Native_option__FStar_Bytes_bytes){ .tag = FStar_Pervasives_Native_None });
