...
TLS_Curve25519_scalarmult(FStar_Bytes_bytes secret1, FStar_Bytes_bytes point)
{
  ...
  uint8_t *pb = alloca(lp * sizeof (uint8_t));
  memset(pb, 0U, lp * sizeof pb[0U]);
  KRML_CHECK_SIZE(sizeof (uint8_t), ls);
  uint8_t *sb = alloca(ls * sizeof (uint8_t));
  ...
  return FStar_Bytes_of_buffer((uint32_t)32U, out);
}
    ...
    default:
      {
        ...
        uint8_t *xb = alloca(l * sizeof (uint8_t));
        memset(xb, 0U, l * sizeof xb[0U]);
        KRML_CHECK_SIZE(sizeof (uint8_t), l);
        uint8_t *yb = alloca(l * sizeof (uint8_t));
        memset(yb, 0U, l * sizeof yb[0U]);
        EverCrypt__ecdh_state *st = EverCrypt_ecdh_load_curve(g1);
        EverCrypt_ecdh_keygen(st, xb, yb);
        FStar_Bytes_bytes uu____0 = FStar_Bytes_of_buffer(l, xb);
        ECGroup_point p1 = { .ecx = uu____0, .ecy = FStar_Bytes_of_buffer(l, yb) };
        return
          (
            (ECGroup_pre_keyshare){
              .tag = ECGroup_KS_CC,
    ...
    default:
      {
        if (gx.tag == ECGroup_KS_CC)
        {
          ...
          if (gy.tag == ECGroup_S_CC)
          {
            ...
            memset(yb, 0U, l * sizeof yb[0U]);
            KRML_CHECK_SIZE(sizeof (uint8_t), l);
            uint8_t *rb = alloca(l * sizeof (uint8_t));
            ...
            FStar_Bytes_bytes r = FStar_Bytes_of_buffer(ol, rb);
...
static FStar_Bytes_bytes
FStar_Pervasives_Native_fst__FStar_Bytes_bytes_EverCrypt__dh_state_(
  K___FStar_Bytes_bytes_EverCrypt__dh_state_ x
)
{
  return x.fst;
}
...
static FStar_Bytes_bytes
DHGroup_pubshare(DHGroup_group g1, K___FStar_Bytes_bytes_EverCrypt__dh_state_ k1)
{
  return FStar_Pervasives_Native_fst__FStar_Bytes_bytes_EverCrypt__dh_state_(k1);
}

static K___FStar_Bytes_bytes_EverCrypt__dh_state_ DHGroup_keygen(DHGroup_group g1)
{
  ...
  KRML_CHECK_SIZE(sizeof (uint8_t), lp);
  uint8_t *pub = alloca(lp * sizeof (uint8_t));
  memset(pub, 0U, lp * sizeof pub[0U]);
  uint32_t lpub = EverCrypt_dh_keygen(st, pub);
  FStar_Bytes_bytes s = FStar_Bytes_of_buffer(lpub, pub);
  return ((K___FStar_Bytes_bytes_EverCrypt__dh_state_){ .fst = s, .snd = st });
}
...
FStar_List_Tot_Base_mem__Parsers_NamedGroup_namedGroup(
  Parsers_NamedGroup_namedGroup x0,
  Prims_list__Parsers_NamedGroup_namedGroup *uu___7_9530
)
{
  Parsers_NamedGroup_namedGroup x = x0;
  Prims_list__Parsers_NamedGroup_namedGroup *uu____0 = uu___7_9530;
  while (true)
    if (uu____0->tag == Prims_Nil)
      return false;
    else if (uu____0->tag == Prims_Cons)
    {
      Prims_list__Parsers_NamedGroup_namedGroup *tl1 = uu____0->tl;
      Parsers_NamedGroup_namedGroup hd1 = uu____0->hd;
...
CommonDH_pre_share_ CommonDH_pre_pubshare(CommonDH_group_ g1, CommonDH_pre_keyshare_ ks)
{
  if (g1.tag == CommonDH_FFDH)
  {
    ...
    {
      K___FStar_Bytes_bytes_EverCrypt__dh_state_ ks1 = ks.val.case_KS_FF._1;
      return
        (
          (CommonDH_pre_share_){
            .tag = CommonDH_S_FF,
...
CommonDH_raw_dh_initiator(CommonDH_group_ g1, CommonDH_pre_keyshare_ x, CommonDH_pre_share_ gy)
{
  ...
  if (g1.tag == CommonDH_FFDH)
  {
    ...
    {
      K___FStar_Bytes_bytes_EverCrypt__dh_state_ x1 = x.val.case_KS_FF._1;
...
*/
static FStar_Bytes_bytes
CommonDH_keyShareEntriesBytes_aux(
  FStar_Bytes_bytes b0,
  Prims_list__CommonDH_keyShareEntry *kes0
)
{
  FStar_Bytes_bytes b = b0;
  Prims_list__CommonDH_keyShareEntry *kes = kes0;
  while (true)
...
CommonDH_parseKeyShareEntries_aux(
  FStar_Bytes_bytes b0,
  Prims_list__CommonDH_keyShareEntry *entries0
)
{
  FStar_Bytes_bytes b = b0;
  Prims_list__CommonDH_keyShareEntry *entries = entries0;
  while (true)
        ...
        else if (scrut0.tag == FStar_Error_Correct)
        {
          ...
          else if (scrut.tag == FStar_Error_Correct)
          {
            ...
            *buf0 = KRML_HOST_MALLOC(sizeof (Prims_list__CommonDH_keyShareEntry));
            ...
            entries = FStar_List_Tot_Base_append__CommonDH_keyShareEntry(entries, buf0);
          }
          else
            return
              KRML_EABORT(FStar_Error_optResult__K___Parsers_Alert_alert_Prims_string_Prims_list__CommonDH_keyShareEntry,
                "unreachable (pattern matches are exhaustive in F*)");
        }
        else
          return
...
*/
FStar_Bytes_bytes CommonDH_keyShareBytes(CommonDH_keyShare uu___1_4861)
{
  if (uu___1_4861.tag == CommonDH_ClientKeyShare)
  {
    Prims_list__CommonDH_keyShareEntry *cks = uu___1_4861.val.case_ClientKeyShare;
    return CommonDH_clientKeyShareBytes(cks);
  }
  else if (uu___1_4861.tag == CommonDH_ServerKeyShare)
  {
    CommonDH_keyShareEntry sks = uu___1_4861.val.case_ServerKeyShare;
    return CommonDH_serverKeyShareBytes(sks);
  }
  else if (uu___1_4861.tag == CommonDH_HRRKeyShare)
  {
    Parsers_NamedGroup_namedGroup ng = uu___1_4861.val.case_HRRKeyShare;
