...
TLS_ioresult_w
TLS_writeHandshake(Connection_connection c0, FStar_Pervasives_Native_option__bool new_writer0)
{
  Connection_connection c = c0;
  FStar_Pervasives_Native_option__bool new_writer = new_writer0;
  while (true)
  {
    TLSInfo_id i1 = TLS_currentId(c, TLSConstants_Writer);
    FStar_Pervasives_Native_option__StAE_state wopt = TLS_current_writer(c, i1);
    ...
    else if (scrut0.tag == FStar_Error_Correct)
    {
      ...
      return KRML_EABORT(TLS_ioresult_w, "unreachable (pattern matches are exhaustive in F*)");
  }
  KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
    __FILE__,
    __LINE__,
...
Prims_string TLS_string_of_ioresult_i(TLSInfo_id i1, TLS_ioresult_i uu___1_3983)
{
  if (uu___1_3983.tag == TLS_ReadWouldBlock)
    return "ReadWouldBlock";
  else if (uu___1_3983.tag == TLS_Read && uu___1_3983.val.case_Read.tag == DataStream_Data)
  {
    FStar_Bytes_bytes d = uu___1_3983.val.case_Read.val.case_Data;
    return
      Prims_strcat("Read ",
  ...
  else if (uu___1_3983.tag == TLS_Read && uu___1_3983.val.case_Read.tag == DataStream_Close)
    return "Read Close";
  else if (uu___1_3983.tag == TLS_Read && uu___1_3983.val.case_Read.tag == DataStream_Alert)
  {
    Parsers_Alert_alert a = uu___1_3983.val.case_Read.val.case_Alert;
  ...
  (
    uu___1_3983.tag
    == TLS_ReadError
    && uu___1_3983.val.case_ReadError.o.tag == FStar_Pervasives_Native_Some
  )
  {
    Prims_string txt = uu___1_3983.val.case_ReadError.txt;
    Parsers_Alert_alert o = uu___1_3983.val.case_ReadError.o.v;
    return
  ...
  (
    uu___1_3983.tag
    == TLS_ReadError
    && uu___1_3983.val.case_ReadError.o.tag == FStar_Pervasives_Native_None
  )
  {
    Prims_string txt = uu___1_3983.val.case_ReadError.txt;
    return Prims_strcat("ReadError ", txt);
  }
  else if (uu___1_3983.tag == TLS_CertQuery)
    return "CertQuery";
  else if (uu___1_3983.tag == TLS_Update)
  {
    bool b = uu___1_3983.val.case_Update;
    Prims_string ite;
    if (b)
      ite = "writable";
    else
  ...
  else if (uu___1_3983.tag == TLS_Complete)
    return "Complete";
  else if (uu___1_3983.tag == TLS_ReadAgain)
    return "ReadAgain";
  else if (uu___1_3983.tag == TLS_ReadAgainFinishing)
...
FStar_Error_optResult__K___Parsers_Alert_alert_Prims_string_FStar_Pervasives_Native_option__Content_fragment
TLS_readFragment(Connection_connection c0, TLSInfo_id i10)
{
  Connection_connection c = c0;
  TLSInfo_id i1 = i10;
  while (true)
      ...
      else
      {
        ...
        else
        {
          ...
          else if (scrut.tag == FStar_Pervasives_Native_Some)
          {
            ...
            return
              KRML_EABORT(FStar_Error_optResult__K___Parsers_Alert_alert_Prims_string_FStar_Pervasives_Native_option__Content_fragment,
                "unreachable (pattern matches are exhaustive in F*)");
        }
      }
    }
...
TLS_ioresult_i TLS_read(Connection_connection c0, TLSInfo_id i10)
{
  Connection_connection c = c0;
  TLSInfo_id i1 = i10;
  while (true)
  {
    ...
    if (r.tag == TLS_WriteError)
    {
      ...
      return KRML_EABORT(TLS_ioresult_i, "Sent Close");
      ...
      else
      {
        ...
        else if (result.tag == TLS_ReadAgain)
        {
          ...
          else if (result.tag == TLS_Read)
          {
            ...
            return
              KRML_EABORT(TLS_ioresult_i,
                "unreachable (pattern matches are exhaustive in F*)");
      }
    }
    else
      return KRML_EABORT(TLS_ioresult_i, "unreachable (pattern matches are exhaustive in F*)");
  }
  KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
    __FILE__,
    __LINE__,
