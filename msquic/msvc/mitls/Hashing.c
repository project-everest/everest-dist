...
FStar_Bytes_bytes Hashing_Spec_emptyHash(Spec_Hash_Definitions_hash_alg uu___0_87)
{
  switch (uu___0_87)
  {
    case Spec_Hash_Definitions_MD5:
      {
        return FStar_Bytes_bytes_of_hex("d41d8cd98f00b204e9800998ecf8427e");
      }
    case Spec_Hash_Definitions_SHA1:
      {
        return FStar_Bytes_bytes_of_hex("da39a3ee5e6b4b0d3255bfef95601890afd80709");
      }
    case Spec_Hash_Definitions_SHA2_224:
      {
        return FStar_Bytes_bytes_of_hex("2bc9476102bb288234c415a2b01f828ea62ac5b3e42f");
      }
    case Spec_Hash_Definitions_SHA2_256:
      {
        return
          FStar_Bytes_bytes_of_hex("e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855");
      }
    case Spec_Hash_Definitions_SHA2_384:
      {
        return
          FStar_Bytes_bytes_of_hex("38b060a751ac96384cd9327eb1b1e36a21fdb71114be07434c0cc7bf63f6e1da274edebfe76f65fbd51ad2f14898b95b");
      }
    case Spec_Hash_Definitions_SHA2_512:
      {
        return
          FStar_Bytes_bytes_of_hex("cf83e1357eefb8bdf1542850d66d8007d620e4050b5715dc83f4a921d36ce9ce47d0d13c5d85f2b0ff8318d2877eec2f63b931bd47417a81a538327af927da3e");
      }
    default:
      {
...
FStar_Bytes_bytes Hashing_Spec_zeroHash(Spec_Hash_Definitions_hash_alg a)
{
  uint32_t sw;
  switch (a)
  {
  ...
  return FStar_Bytes_create(sw, (uint8_t)0U);
}

uint32_t Hashing_Spec_tls_tagLen(Hashing_Spec_tls_alg uu___1_169)
{
  if (uu___1_169.tag == Hashing_Spec_Hash)
  {
    Spec_Hash_Definitions_hash_alg a = uu___1_169._0;
    switch (a)
    {
      case Spec_Hash_Definitions_MD5:
        {
  ...
  else if (uu___1_169.tag == Hashing_Spec_MD5SHA1)
    return (uint32_t)36U;
  else
  {
...
FStar_Bytes_bytes
HMAC_hmac(Spec_Hash_Definitions_hash_alg a, FStar_Bytes_bytes k1, FStar_Bytes_bytes m)
{
  uint32_t lt1;
  switch (a)
  {
  ...
  uint32_t lk = FStar_Bytes_len(k1);
  KRML_CHECK_SIZE(sizeof (uint8_t), lk);
  uint8_t *bk = alloca(lk * sizeof (uint8_t));
  memset(bk, 0U, lk * sizeof bk[0U]);
  FStar_Bytes_store_bytes(k1, bk);
  KRML_CHECK_SIZE(sizeof (uint8_t), lt1);
  uint8_t *bt = alloca(lt1 * sizeof (uint8_t));
  memset(bt, 0U, lt1 * sizeof bt[0U]);
  uint32_t lm = FStar_Bytes_len(m);
  if (lm == (uint32_t)0U)
  {
    uint8_t *bm = NULL;
    EverCrypt_HMAC_compute(a, bt, bk, lk, bm, lm);
  }
  else
  {
    KRML_CHECK_SIZE(sizeof (uint8_t), lm);
    uint8_t *bm = alloca(lm * sizeof (uint8_t));
    memset(bm, 0U, lm * sizeof bm[0U]);
    FStar_Bytes_store_bytes(m, bm);
    EverCrypt_HMAC_compute(a, bt, bk, lk, bm, lm);
  }
  FStar_Bytes_bytes t1 = FStar_Bytes_of_buffer(lt1, bt);
  return t1;
}
...
HMAC_hmacVerify(
  ...
  FStar_Bytes_bytes k1,
  FStar_Bytes_bytes p1,
  FStar_Bytes_bytes t1
)
{
  FStar_Bytes_bytes result = HMAC_hmac(a, k1, p1);
  return __eq__FStar_Bytes_bytes(result, t1);
}

FStar_Bytes_bytes
HMAC_tls_mac(Spec_Hash_Definitions_hash_alg a, FStar_Bytes_bytes k1, FStar_Bytes_bytes msg)
{
  return HMAC_hmac(a, k1, msg);
}

FStar_Bytes_bytes
HKDF_expand(
  Spec_Hash_Definitions_hash_alg ha,
  FStar_Bytes_bytes prk,
  FStar_Bytes_bytes info,
  uint32_t len1
)
{
  uint32_t tlen;
  ...
  uint8_t *prk_p = alloca(tlen * sizeof (uint8_t));
  memset(prk_p, 0U, tlen * sizeof prk_p[0U]);
  FStar_Bytes_store_bytes(prk, prk_p);
  KRML_CHECK_SIZE(sizeof (uint8_t), len1);
  uint8_t *tag_p = alloca(len1 * sizeof (uint8_t));
  memset(tag_p, 0U, len1 * sizeof tag_p[0U]);
  uint32_t infolen = FStar_Bytes_len(info);
  if (infolen == (uint32_t)0U)
  {
    uint8_t *info_p = NULL;
    EverCrypt_HKDF_hkdf_expand(ha, tag_p, prk_p, tlen, info_p, infolen, len1);
  }
  else
  {
    KRML_CHECK_SIZE(sizeof (uint8_t), infolen);
    uint8_t *info_p = alloca(infolen * sizeof (uint8_t));
    memset(info_p, 0U, infolen * sizeof info_p[0U]);
    FStar_Bytes_store_bytes(info, info_p);
    EverCrypt_HKDF_hkdf_expand(ha, tag_p, prk_p, tlen, info_p, infolen, len1);
  }
  FStar_Bytes_bytes tag = FStar_Bytes_of_buffer(len1, tag_p);
  return tag;
}

FStar_Bytes_bytes HKDF_tls13_prefix;
...
{
  FStar_Bytes_bytes
  label_bytes = FStar_Bytes_append(HKDF_tls13_prefix, FStar_Bytes_bytes_of_string(label));
  FStar_Bytes_bytes
  info =
    Parsers_HKDF_HkdfLabel_hkdfLabel_serializer32((
        (Parsers_HKDF_HkdfLabel_hkdfLabel){
          .length = (uint16_t)len1,
          .label = label_bytes,
          .context = digest
        }
      ));
  return HKDF_expand(ha, secret1, info, len1);
}
...
)
{
  ...
  return HKDF_expand_label(ha, secret1, label, digest, len1);
}

FStar_Bytes_bytes Hashing_compute(Spec_Hash_Definitions_hash_alg a, FStar_Bytes_bytes text)
{
  uint32_t tlen;
  switch (a)
  {
  ...
  KRML_CHECK_SIZE(sizeof (uint8_t), tlen);
  uint8_t *output = alloca(tlen * sizeof (uint8_t));
  memset(output, 0U, tlen * sizeof output[0U]);
  uint32_t len1 = FStar_Bytes_len(text);
  if (len1 == (uint32_t)0U)
  {
    uint8_t *input = NULL;
    EverCrypt_Hash_hash(a, output, input, len1);
  }
  else
  {
    KRML_CHECK_SIZE(sizeof (uint8_t), len1);
    uint8_t *input = alloca(len1 * sizeof (uint8_t));
    memset(input, 0U, len1 * sizeof input[0U]);
    FStar_Bytes_store_bytes(text, input);
    EverCrypt_Hash_hash(a, output, input, len1);
  }
  FStar_Bytes_bytes t1 = FStar_Bytes_of_buffer(tlen, output);
  return t1;
}

#define Hashing_Inputs 0

typedef uint8_t Hashing_accv__tags;

FStar_Bytes_bytes Hashing_content(Spec_Hash_Definitions_hash_alg a, FStar_Bytes_bytes v1)
{
  return v1;
}

FStar_Bytes_bytes Hashing_start(Spec_Hash_Definitions_hash_alg a)
{
...
Hashing_extend(
  Spec_Hash_Definitions_hash_alg a,
  FStar_Bytes_bytes uu____429,
  FStar_Bytes_bytes b1
)
{
  FStar_Bytes_bytes b0 = uu____429;
  return FStar_Bytes_append(b0, b1);
}

FStar_Bytes_bytes
Hashing_finalize(Spec_Hash_Definitions_hash_alg a, FStar_Bytes_bytes uu____458)
{
  FStar_Bytes_bytes b = uu____458;
  return Hashing_compute(a, b);
}

FStar_Bytes_bytes Hashing_compute_MD5SHA1(FStar_Bytes_bytes data)
{
  FStar_Bytes_bytes uu____0 = Hashing_compute(Spec_Hash_Definitions_MD5, data);
  return FStar_Bytes_append(uu____0, Hashing_compute(Spec_Hash_Definitions_SHA1, data));
}
