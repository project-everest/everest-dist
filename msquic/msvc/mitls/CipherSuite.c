...
*FStar_List_Tot_Base_rev_acc__CipherSuite_cipherSuite_(
  ...
  Prims_list__CipherSuite_cipherSuite_ *acc0
)
{
  Prims_list__CipherSuite_cipherSuite_ *l = l0;
  Prims_list__CipherSuite_cipherSuite_ *acc = acc0;
  while (true)
    if (l->tag == Prims_Nil)
      return acc;
  ...
  while (true)
    ...
    else if (l->tag == Prims_Cons)
    {
      ...
      else if (scrut.tag == FStar_Pervasives_Native_Some)
      {
        ...
        *buf = KRML_HOST_MALLOC(sizeof (Prims_list__CipherSuite_cipherSuite_));
        buf[0U]
        = ((Prims_list__CipherSuite_cipherSuite_){ .tag = Prims_Cons, .hd = c, .tl = accu });
        accu = buf;
      }
      else
        return KRML_EABORT(void *, "unreachable (pattern matches are exhaustive in F*)");
...
*FStar_List_Tot_Base_rev_acc__Parsers_CipherSuite_cipherSuite(
  Prims_list__Parsers_CipherSuite_cipherSuite *l0,
  Prims_list__Parsers_CipherSuite_cipherSuite *acc0
)
{
  Prims_list__Parsers_CipherSuite_cipherSuite *l = l0;
  Prims_list__Parsers_CipherSuite_cipherSuite *acc = acc0;
  while (true)
    if (l->tag == Prims_Nil)
      return acc;
    else if (l->tag == Prims_Cons)
...
{
  Prims_list__CipherSuite_cipherSuite_ *l = l0;
  Prims_list__Parsers_CipherSuite_cipherSuite *accu = accu0;
  while (true)
    if (l->tag == Prims_Nil)
