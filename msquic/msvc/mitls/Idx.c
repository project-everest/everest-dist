...
{
  if (projectee.tag == Idx_ExtractDH)
    return true;
  else
    return false;
}
...
{
  if (projectee.tag == Idx_ExtractDH)
    return projectee.val.case_ExtractDH;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
{
  if (projectee.tag == Idx_ExpandLog)
    return true;
  else
    return false;
}
...
{
  if (projectee.tag == Idx_ExpandLog)
    return projectee.val.case_ExpandLog.info;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
{
  if (projectee.tag == Idx_ExpandLog)
    return projectee.val.case_ExpandLog.hv;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
{
  if (projectee->tag == Idx_Preshared)
    return true;
  else
    return false;
}
...
{
  if (projectee->tag == Idx_Preshared)
    return projectee->val.case_Preshared.a;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
{
  if (projectee->tag == Idx_Preshared)
    return projectee->val.case_Preshared._1;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
{
  if (projectee->tag == Idx_Derive)
    return projectee->val.case_Derive.i;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
{
  if (projectee->tag == Idx_Derive)
    return projectee->val.case_Derive.l;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
{
  if (projectee->tag == Idx_Derive)
    return projectee->val.case_Derive._2;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
{
  Idx_pre_id *uu____0 = uu___0_3310;
  while (true)
    if (uu____0->tag == Idx_Preshared)
      return uu____0->val.case_Preshared.a;
    else if (uu____0->tag == Idx_Derive)
    {
      Idx_pre_id *i1 = uu____0->val.case_Derive.i;
      uu____0 = i1;
    }
    else
      return
        KRML_EABORT(Spec_Hash_Definitions_hash_alg,
          "unreachable (pattern matches are exhaustive in F*)");
  KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
    __FILE__,
    __LINE__,
...
void *Idx_honesty_table = (void *)(uint8_t)0U;
