...
static bool RSAKey_honest(FStar_Bytes_bytes pk)
{
  return false;
}

static bool RSAKey_strong(Parsers_ProtocolVersion_protocolVersion pv)
{
  return false;
}

FStar_Bytes_bytes TLSPRF_coerce(FStar_Bytes_bytes k1)
{
  return k1;
}
...
TLSPRF_p_hash_int(
  Spec_Hash_Definitions_hash_alg alg0,
  FStar_Bytes_bytes secret10,
  FStar_Bytes_bytes seed0,
  uint32_t len10,
  uint32_t it0,
  FStar_Bytes_bytes aPrev0,
  FStar_Bytes_bytes acc0
)
{
  Spec_Hash_Definitions_hash_alg alg = alg0;
  FStar_Bytes_bytes secret1 = secret10;
  FStar_Bytes_bytes seed = seed0;
  uint32_t len1 = len10;
  uint32_t it = it0;
  FStar_Bytes_bytes aPrev = aPrev0;
  FStar_Bytes_bytes acc = acc0;
  while (true)
  {
    FStar_Bytes_bytes aCur = HMAC_tls_mac(alg, secret1, aPrev);
    FStar_Bytes_bytes pCur = HMAC_tls_mac(alg, secret1, FStar_Bytes_append(aCur, seed));
    if (it == (uint32_t)1U)
    {
      uint32_t hs = CipherSuite_macSize(alg);
      uint32_t r = len1 % hs;
      K___FStar_Bytes_bytes_FStar_Bytes_bytes scrut = FStar_Bytes_split(pCur, r);
      FStar_Bytes_bytes pCur1 = scrut.fst;
      return FStar_Bytes_append(acc, pCur1);
    }
    else
    {
      it = it - (uint32_t)1U;
      aPrev = aCur;
      acc = FStar_Bytes_append(acc, pCur);
    }
  }
...
TLSPRF_p_hash(
  ...
  FStar_Bytes_bytes seed,
  uint32_t len1
)
{
  uint32_t hs = CipherSuite_macSize(alg);
  uint32_t it = len1 / hs + (uint32_t)1U;
  FStar_Bytes_bytes
  r = TLSPRF_p_hash_int(alg, secret1, seed, len1, it, seed, FStar_Bytes_empty_bytes);
  return r;
}
...
{
  Prims_int l_s = FStar_UInt32_v(FStar_Bytes_len(secret1));
  Prims_int
  l_s1 = Prims_op_Division(Prims_op_Addition(l_s, (krml_checked_int_t)1), (krml_checked_int_t)2);
  K___FStar_Bytes_bytes_FStar_Bytes_bytes
  scrut = FStar_Bytes_split(secret1, FStar_UInt32_uint_to_t(l_s1));
  FStar_Bytes_bytes secret11 = scrut.fst;
  FStar_Bytes_bytes secret2 = scrut.snd;
  FStar_Bytes_bytes newseed = FStar_Bytes_append(label, seed);
  FStar_Bytes_bytes hmd5 = TLSPRF_p_hash(Spec_Hash_Definitions_MD5, secret11, newseed, len1);
  FStar_Bytes_bytes hsha1 = TLSPRF_p_hash(Spec_Hash_Definitions_SHA1, secret2, newseed, len1);
  return FStar_Bytes_xor(len1, hmd5, hsha1);
}

FStar_Bytes_bytes TLSPRF_tls_client_label;

FStar_Bytes_bytes TLSPRF_tls_server_label;

static FStar_Bytes_bytes TLSPRF_tls_finished_label(TLSConstants_role uu___0_230)
{
  switch (uu___0_230)
  {
    case TLSConstants_Client:
      {
        return TLSPRF_tls_client_label;
      }
    case TLSConstants_Server:
      {
        return TLSPRF_tls_server_label;
      }
    default:
      {
...
static uint32_t TLSPRF_verifyDataLen = (uint32_t)12U;
...
{
  Spec_Hash_Definitions_hash_alg prfMacAlg = TLSConstants_prfMacAlg_of_ciphersuite(cs);
  return TLSPRF_p_hash(prfMacAlg, ms, FStar_Bytes_append(label, data), len1);
}

static FStar_Bytes_bytes
TLSPRF_tls12prf_(
  Spec_Hash_Definitions_hash_alg macAlg,
  FStar_Bytes_bytes ms,
  FStar_Bytes_bytes label,
  FStar_Bytes_bytes data,
  uint32_t len1
...
{
  return TLSPRF_p_hash(macAlg, ms, FStar_Bytes_append(label, data), len1);
}

FStar_Bytes_bytes
TLSPRF_finished12(
  Spec_Hash_Definitions_hash_alg hAlg,
  FStar_Bytes_bytes ms,
  TLSConstants_role role,
  FStar_Bytes_bytes log
)
{
  return
    TLSPRF_p_hash(hAlg,
      ms,
      FStar_Bytes_append(TLSPRF_tls_finished_label(role), log),
      TLSPRF_verifyDataLen);
}

FStar_Bytes_bytes
TLSPRF_prf(
  K___Parsers_ProtocolVersion_protocolVersion_CipherSuite_cipherSuite_ uu____511,
  FStar_Bytes_bytes secret1,
  FStar_Bytes_bytes label,
  FStar_Bytes_bytes data,
  uint32_t len1
...
{
  Parsers_ProtocolVersion_protocolVersion pv = uu____511.fst;
  CipherSuite_cipherSuite_ cs = uu____511.snd;
  if (pv.tag == Parsers_ProtocolVersion_TLS_1p0)
    return TLSPRF_tls_prf(secret1, label, data, len1);
  else if (pv.tag == Parsers_ProtocolVersion_TLS_1p1)
    return TLSPRF_tls_prf(secret1, label, data, len1);
  else if (pv.tag == Parsers_ProtocolVersion_TLS_1p2)
    return TLSPRF_tls12prf(cs, secret1, label, data, len1);
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
static FStar_Bytes_bytes
TLSPRF_prf_(
  TLSConstants_prePrfAlg a,
  FStar_Bytes_bytes secret1,
  FStar_Bytes_bytes data,
  uint32_t len1
)
{
  if (a.tag == TLSConstants_PRF_TLS_1p2)
  {
    Spec_Hash_Definitions_hash_alg macAlg = a.val.case_PRF_TLS_1p2._1;
    FStar_Bytes_bytes label = a.val.case_PRF_TLS_1p2._0;
    return TLSPRF_tls12prf_(macAlg, secret1, label, data, len1);
  }
  else if (a.tag == TLSConstants_PRF_TLS_1p01)
  {
    FStar_Bytes_bytes label = a.val.case_PRF_TLS_1p01;
    return TLSPRF_tls_prf(secret1, label, data, len1);
  }
  else
  {
...
{
  return TLSPRF_prf_(a, secret1, data, len1);
}
...
bool PMS_ideal = false;

FStar_Bytes_bytes PMS___proj__Mkrsaseed__item__seed(FStar_Bytes_bytes projectee)
{
  return projectee;
}

bool PMS_uu___is_IdealRSAPMS(PMS_rsapms projectee)
{
  if (projectee.tag == PMS_IdealRSAPMS)
    return true;
  else
    return false;
}

FStar_Bytes_bytes PMS___proj__IdealRSAPMS__item___0(PMS_rsapms projectee)
{
  if (projectee.tag == PMS_IdealRSAPMS)
    return projectee.val.case_IdealRSAPMS;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
bool PMS_uu___is_ConcreteRSAPMS(PMS_rsapms projectee)
{
  if (projectee.tag == PMS_ConcreteRSAPMS)
    return true;
  else
    return false;
}

FStar_Bytes_bytes PMS___proj__ConcreteRSAPMS__item___0(PMS_rsapms projectee)
{
  if (projectee.tag == PMS_ConcreteRSAPMS)
    return projectee.val.case_ConcreteRSAPMS;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
bool
PMS_honestRSAPMS(
  FStar_Bytes_bytes pk,
  Parsers_ProtocolVersion_protocolVersion cv,
  PMS_rsapms pms
)
{
  if (pms.tag == PMS_IdealRSAPMS)
    return true;
  else if (pms.tag == PMS_ConcreteRSAPMS)
    return false;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
...
PMS_rsapms PMS_genRSA(FStar_Bytes_bytes pk, Parsers_ProtocolVersion_protocolVersion vc)
{
  FStar_Bytes_bytes verBytes = TLSConstants_versionBytes(vc);
  FStar_Bytes_bytes rnd = Random_sample32((uint32_t)46U);
  FStar_Bytes_bytes pms = FStar_Bytes_append(verBytes, rnd);
  if (PMS_ideal && RSAKey_honest(pk) && RSAKey_strong(vc))
    return ((PMS_rsapms){ .tag = PMS_IdealRSAPMS, .val = { .case_IdealRSAPMS = pms } });
  else
    return ((PMS_rsapms){ .tag = PMS_ConcreteRSAPMS, .val = { .case_ConcreteRSAPMS = pms } });
}
...
{
  return ((PMS_rsapms){ .tag = PMS_ConcreteRSAPMS, .val = { .case_ConcreteRSAPMS = b } });
}

FStar_Bytes_bytes
PMS_leakRSA(FStar_Bytes_bytes pk, Parsers_ProtocolVersion_protocolVersion cv, PMS_rsapms pms)
{
  if (pms.tag == PMS_IdealRSAPMS)
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n", __FILE__, __LINE__, "pms is dishonest");
    KRML_HOST_EXIT(255U);
  }
  else if (pms.tag == PMS_ConcreteRSAPMS)
    return pms.val.case_ConcreteRSAPMS;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
FStar_Bytes_bytes PMS___proj__Mkdhseed__item__seed(FStar_Bytes_bytes projectee)
{
  return projectee;
}

bool PMS_uu___is_IdealDHPMS(PMS_dhpms projectee)
{
  if (projectee.tag == PMS_IdealDHPMS)
    return true;
  else
    return false;
}

FStar_Bytes_bytes PMS___proj__IdealDHPMS__item___0(PMS_dhpms projectee)
{
  if (projectee.tag == PMS_IdealDHPMS)
    return projectee.val.case_IdealDHPMS;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
bool PMS_uu___is_ConcreteDHPMS(PMS_dhpms projectee)
{
  if (projectee.tag == PMS_ConcreteDHPMS)
    return true;
  else
    return false;
}

FStar_Bytes_bytes PMS___proj__ConcreteDHPMS__item___0(PMS_dhpms projectee)
{
  if (projectee.tag == PMS_ConcreteDHPMS)
    return projectee.val.case_ConcreteDHPMS;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
PMS_honestDHPMS(
  CommonDH_group_ g1,
  CommonDH_pre_share_ gx,
  CommonDH_pre_share_ gy,
  PMS_dhpms pms
)
{
  if (pms.tag == PMS_IdealDHPMS)
    return true;
  else if (pms.tag == PMS_ConcreteDHPMS)
    return false;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
...
{
  return ((PMS_dhpms){ .tag = PMS_ConcreteDHPMS, .val = { .case_ConcreteDHPMS = b } });
}

bool PMS_uu___is_RSAPMS(PMS_pms projectee)
{
  if (projectee.tag == PMS_RSAPMS)
    return true;
  else
    return false;
}

K___FStar_Bytes_bytes_Parsers_ProtocolVersion_protocolVersion_PMS_rsapms
PMS___proj__RSAPMS__item___0(PMS_pms projectee)
{
  if (projectee.tag == PMS_RSAPMS)
    return projectee.val.case_RSAPMS;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
bool PMS_uu___is_DHPMS(PMS_pms projectee)
{
  if (projectee.tag == PMS_DHPMS)
    return true;
  else
    return false;
}

CommonDH_group_ PMS___proj__DHPMS__item__g(PMS_pms projectee)
{
  if (projectee.tag == PMS_DHPMS)
    return projectee.val.case_DHPMS.g;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
CommonDH_pre_share_ PMS___proj__DHPMS__item__gx(PMS_pms projectee)
{
  if (projectee.tag == PMS_DHPMS)
    return projectee.val.case_DHPMS.gx;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
CommonDH_pre_share_ PMS___proj__DHPMS__item___2(PMS_pms projectee)
{
  if (projectee.tag == PMS_DHPMS)
    return projectee.val.case_DHPMS._2;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
PMS_dhpms PMS___proj__DHPMS__item___3(PMS_pms projectee)
{
  if (projectee.tag == PMS_DHPMS)
    return projectee.val.case_DHPMS._3;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
      __FILE__,
...
bool PMS_uu___is_DummyPMS(PMS_pms projectee)
{
  if (projectee.tag == PMS_DummyPMS)
    return true;
  else
    return false;
}

bool PMS_honestPMS(PMS_pms uu___0_635)
{
  if (uu___0_635.tag == PMS_RSAPMS)
  {
    PMS_rsapms pms = uu___0_635.val.case_RSAPMS.thd;
    Parsers_ProtocolVersion_protocolVersion pv = uu___0_635.val.case_RSAPMS.snd;
    FStar_Bytes_bytes pk = uu___0_635.val.case_RSAPMS.fst;
    return PMS_honestRSAPMS(pk, pv, pms);
  }
  else if (uu___0_635.tag == PMS_DHPMS)
  {
    PMS_dhpms pms = uu___0_635.val.case_DHPMS._3;
    CommonDH_pre_share_ gy = uu___0_635.val.case_DHPMS._2;
    CommonDH_pre_share_ gx = uu___0_635.val.case_DHPMS.gx;
    CommonDH_group_ g1 = uu___0_635.val.case_DHPMS.g;
    return PMS_honestDHPMS(g1, gx, gy, pms);
  }
  else if (uu___0_635.tag == PMS_DummyPMS)
    return false;
  else
  {
    KRML_HOST_EPRINTF("KreMLin abort at %s:%d\n%s\n",
