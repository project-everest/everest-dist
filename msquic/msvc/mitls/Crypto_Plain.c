...
static Prims_list__uint8_t *FStar_Seq_Base_empty__uint8_t()
{
  KRML_CHECK_SIZE(sizeof (Prims_list__uint8_t), (uint32_t)1U);
  Prims_list__uint8_t *buf = KRML_HOST_MALLOC(sizeof (Prims_list__uint8_t));
  buf[0U] = ((Prims_list__uint8_t){ .tag = Prims_Nil });
  return buf;
}

static Prims_list__uint8_t
    ...
    =
      (
        (Prims_list__uint8_t){
          .tag = Prims_Cons,
          .hd = a,
          .tl = FStar_List_Tot_Base_append__uint8_t(tl1, y)
        }
      );
    return buf;
  }
...
static Prims_list__uint8_t
*FStar_Seq_Base_append__uint8_t(Prims_list__uint8_t *s1, Prims_list__uint8_t *s2)
{
  return FStar_List_Tot_Base_append__uint8_t(s1, s2);
}

static Prims_list__uint8_t *FStar_Seq_Base_cons__uint8_t(uint8_t x, Prims_list__uint8_t *s)
{
  KRML_CHECK_SIZE(sizeof (Prims_list__uint8_t), (uint32_t)1U);
  Prims_list__uint8_t *buf = KRML_HOST_MALLOC(sizeof (Prims_list__uint8_t));
  buf[0U] = ((Prims_list__uint8_t){ .tag = Prims_Cons, .hd = x, .tl = s });
  return buf;
}

static Prims_list__uint8_t *FStar_Seq_Base_create__uint8_t(Prims_int len, uint8_t v)
{
  if (len == (krml_checked_int_t)0)
  {
    ...
    return
      FStar_Seq_Base_cons__uint8_t(v,
        FStar_Seq_Base_create__uint8_t(Prims_op_Subtraction(len, (krml_checked_int_t)1), v));
}

static Prims_list__uint8_t
*FStar_Seq_Properties_cons__uint8_t(uint8_t x, Prims_list__uint8_t *s)
{
  return
    FStar_Seq_Base_append__uint8_t(FStar_Seq_Base_create__uint8_t((krml_checked_int_t)1, x),
      s);
}

static Prims_list__uint8_t *Crypto_Symmetric_Bytes_load_bytes(uint32_t l, uint8_t *buf)
{
  if (l == (uint32_t)0U)
    return FStar_Seq_Base_empty__uint8_t();
  else
  {
    uint8_t b = buf[0U];
    Prims_list__uint8_t
    *t = Crypto_Symmetric_Bytes_load_bytes(l - (uint32_t)1U, buf + (uint32_t)1U);
    return FStar_Seq_Properties_cons__uint8_t(b, t);
  }
}

static uint8_t FStar_List_Tot_Base_hd__uint8_t(Prims_list__uint8_t *uu___0_39)
{
...
static Prims_list__uint8_t *FStar_List_Tot_Base_tail__uint8_t(Prims_list__uint8_t *uu___1_64)
{
...
static Prims_list__uint8_t
*(*FStar_List_Tot_Base_tl__uint8_t)(Prims_list__uint8_t *x0) =
  FStar_List_Tot_Base_tail__uint8_t;

static uint8_t FStar_List_Tot_Base_index__uint8_t(Prims_list__uint8_t *l0, Prims_int i0)
{
  Prims_list__uint8_t *l = l0;
  Prims_int i = i0;
  while (true)
    if (i == (krml_checked_int_t)0)
      return FStar_List_Tot_Base_hd__uint8_t(l);
    else
    {
      l = FStar_List_Tot_Base_tl__uint8_t(l);
...
static uint8_t FStar_Seq_Base_index__uint8_t(Prims_list__uint8_t *s, Prims_int i)
{
  return FStar_List_Tot_Base_index__uint8_t(s, i);
}

static void
Crypto_Symmetric_Bytes_store_bytes_aux(
  uint32_t len0,
  uint8_t *buf0,
  uint32_t i0,
  Prims_list__uint8_t *b0
)
{
  uint32_t len = len0;
  uint8_t *buf = buf0;
  uint32_t i = i0;
  Prims_list__uint8_t *b = b0;
  while (true)
    if (i < len)
    {
      buf[i] = FStar_Seq_Base_index__uint8_t(b, FStar_UInt32_v(i));
      i = i + (uint32_t)1U;
    }
    else
      return;
...
static void
Crypto_Symmetric_Bytes_store_bytes(uint32_t l, uint8_t *buf, Prims_list__uint8_t *b)
{
  Crypto_Symmetric_Bytes_store_bytes_aux(l, buf, (uint32_t)0U, b);
}

Prims_list__uint8_t *Crypto_Plain_repr(TLSInfo_id i1, Prims_int l, Prims_list__uint8_t *p1)
{
  return p1;
}

Prims_list__uint8_t *Crypto_Plain_make(TLSInfo_id i1, Prims_int l, Prims_list__uint8_t *b)
{
  return b;
}

uint8_t *Crypto_Plain_unsafe_hide_buffer(TLSInfo_id i1, Prims_int l, uint8_t *b)
{
  return b;
}

uint8_t *Crypto_Plain_bufferRepr(TLSInfo_id i1, Prims_int l, uint8_t *b)
{
  return b;
}

uint8_t *Crypto_Plain_sub(TLSInfo_id id1, Prims_int l, uint8_t *b, uint32_t i1, uint32_t len1)
{
  return b + i1;
}

Prims_list__uint8_t *Crypto_Plain_load(TLSInfo_id i1, uint32_t l, uint8_t *buf1)
{
  return Crypto_Symmetric_Bytes_load_bytes(l, buf1);
}

void Crypto_Plain_store(TLSInfo_id i1, uint32_t l, uint8_t *buf1, Prims_list__uint8_t *b)
{
  Crypto_Symmetric_Bytes_store_bytes(l, buf1, b);
}
