...
typedef TLSInfo_logInfo Idx_info;

typedef Prims_string Idx_label;

#define Idx_NoIDH 0
#define Idx_IDH 1

typedef uint8_t Idx_id_dhe_tags;

typedef struct Idx_id_dhe_s
{
  Idx_id_dhe_tags tag;
  Prims_dtuple2__CommonDH_group__CommonDH_pre_share_ gX;
  CommonDH_pre_share_ gY;
}
Idx_id_dhe;

bool Idx_uu___is_NoIDH(Idx_id_dhe projectee);

bool Idx_uu___is_IDH(Idx_id_dhe projectee);

Prims_dtuple2__CommonDH_group__CommonDH_pre_share_
Idx___proj__IDH__item__gX(Idx_id_dhe projectee);

CommonDH_pre_share_ Idx___proj__IDH__item__gY(Idx_id_dhe projectee);

typedef Spec_Hash_Definitions_hash_alg Idx_kdfa;

#define Idx_Extract 0
#define Idx_ExtractDH 1
#define Idx_Expand 2
#define Idx_ExpandLog 3

typedef uint8_t Idx_context_tags;

typedef struct Idx_context_s
{
  Idx_context_tags tag;
  union {
    Idx_id_dhe case_ExtractDH;
    struct 
    {
      TLSInfo_logInfo info;
      FStar_Bytes_bytes hv;
    }
    case_ExpandLog;
  }
  val;
...
bool Idx_uu___is_Extract(Idx_context projectee);

bool Idx_uu___is_ExtractDH(Idx_context projectee);

Idx_id_dhe Idx___proj__ExtractDH__item__v(Idx_context projectee);

bool Idx_uu___is_Expand(Idx_context projectee);

bool Idx_uu___is_ExpandLog(Idx_context projectee);

TLSInfo_logInfo Idx___proj__ExpandLog__item__info(Idx_context projectee);

FStar_Bytes_bytes Idx___proj__ExpandLog__item__hv(Idx_context projectee);

typedef Prims_int Idx_id_psk;

typedef struct Idx_pre_id_s Idx_pre_id;

#define Idx_Preshared 0
#define Idx_Derive 1

typedef uint8_t Idx_pre_id_tags;

typedef struct Idx_pre_id_s
{
  Idx_pre_id_tags tag;
  union {
    struct 
...
bool Idx_uu___is_Preshared(Idx_pre_id *projectee);

Spec_Hash_Definitions_hash_alg Idx___proj__Preshared__item__a(Idx_pre_id *projectee);

Prims_int Idx___proj__Preshared__item___1(Idx_pre_id *projectee);

bool Idx_uu___is_Derive(Idx_pre_id *projectee);

Idx_pre_id *Idx___proj__Derive__item__i(Idx_pre_id *projectee);

Prims_string Idx___proj__Derive__item__l(Idx_pre_id *projectee);

Idx_context Idx___proj__Derive__item___2(Idx_pre_id *projectee);

Spec_Hash_Definitions_hash_alg Idx_ha_of_id(Idx_pre_id *uu___0_3310);

extern Idx_id_dhe Idx_idh_of_log(TLSInfo_logInfo uu____345);

extern TLSInfo_logInfo Idx_summary(FStar_Bytes_bytes uu____356);

typedef void *Idx_id;

typedef bool Prims_dtuple2_____bool;

typedef struct Prims_list__Prims_dtuple2_____bool_s Prims_list__Prims_dtuple2_____bool;

typedef struct Prims_list__Prims_dtuple2_____bool_s
{
  Prims_list__Parsers_NamedGroup_namedGroup_tags tag;
  Prims_dtuple2_____bool hd;
  Prims_list__Prims_dtuple2_____bool *tl;
}
Prims_list__Prims_dtuple2_____bool;

typedef Prims_list__Prims_dtuple2_____bool **Idx_i_honesty_table;

typedef void *Idx_h_table;

extern void *Idx_honesty_table;

typedef void *Idx_regid;

void Idx_lemma_honest_corrupt_st();

void Idx_lemma_corrupt_invariant(Prims_string lbl, Idx_context ctx);

#define __Idx_H_DEFINED
#endif
