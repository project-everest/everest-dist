...
{
  Content_contentType ct1 = uu____2896.fst;
  FStar_Bytes_bytes c = uu____2896.snd;
...
{
  Transport_t tcp = tcp0;
  Record_input_state s = s0;
  while (true)
  {
    uint8_t *header = s.b;
      ...
      if (!(received < waiting))
        if (p1 == Record_headerLen)
        {
          ...
          if (scrut.tag == FStar_Error_Error)
          {
            ...
            return
              ((Record_read_result){ .tag = Record_ReadError, .val = { .case_ReadError = e } });
          }
          else if (scrut.tag == FStar_Error_Correct)
          {
            Prims_int length1 = scrut.val.case_Correct.thd;
              ...
              return
                (
                  (Record_read_result){
                    .tag = Record_Received,
                    .val = {
                      .case_Received = { .ct = ct1, .pv = pv, .b = FStar_Bytes_empty_bytes }
                    }
                  }
                );
            }
            ...
            return
              KRML_EABORT(Record_read_result,
                "unreachable (pattern matches are exhaustive in F*)");
        }
        else
        {
