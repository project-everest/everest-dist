...
{
  if (EverCrypt_AutoConfig2_wants_bcrypt())
  {
    return EverCrypt_BCrypt_random_init();
  }
  else
  {
...
void EverCrypt_random_sample(uint32_t len, uint8_t *out1)
{
  if (EverCrypt_AutoConfig2_wants_bcrypt())
  {
    EverCrypt_BCrypt_random_sample(len, out1);
  }
  else
  {
...
void EverCrypt_random_cleanup()
{
  if (EverCrypt_AutoConfig2_wants_bcrypt())
  {
    EverCrypt_BCrypt_random_cleanup();
  }
  else
  {
...
)
{
  ...
  else if (EverCrypt_AutoConfig2_wants_bcrypt())
  {
    EverCrypt_BCrypt_aes128_gcm_encrypt(key, iv, ad, adlen, plaintext, len, cipher1, tag);
  }
  else
  {
...
)
{
  ...
  {
    return EverCrypt_BCrypt_aes128_gcm_decrypt(key, iv, ad, adlen, plaintext, len, cipher1, tag);
  }
  else
  {
...
)
{
  ...
  {
    EverCrypt_BCrypt_aes256_gcm_encrypt(key, iv, ad, adlen, plaintext, len, cipher1, tag);
  }
  else
  {
...
)
{
  ...
  {
    return EverCrypt_BCrypt_aes256_gcm_decrypt(key, iv, ad, adlen, plaintext, len, cipher1, tag);
  }
  else
  {
...
static bool EverCrypt_uu___is_AEAD_BCRYPT(EverCrypt__aead_state projectee)
{
  if (projectee.tag == EverCrypt_AEAD_BCRYPT)
  {
    return true;
  }
  else
    ...
    case EverCrypt_AES128_GCM:
      {
        ...
        else if (EverCrypt_AutoConfig2_wants_bcrypt())
        {
          st =
            (
              (EverCrypt__aead_state){
                .tag = EverCrypt_AEAD_BCRYPT,
                {
                  .case_AEAD_BCRYPT = EverCrypt_BCrypt_aead_create(EverCrypt_BCrypt_AES128_GCM, k1)
                }
              }
            );
        }
          ...
          st =
            (
              (EverCrypt__aead_state){
                .tag = EverCrypt_AEAD_BCRYPT,
                {
                  .case_AEAD_BCRYPT = EverCrypt_BCrypt_aead_create(EverCrypt_BCrypt_AES256_GCM, k1)
                }
              }
            );
        }
...
)
{
  ...
  else if (true && EverCrypt_uu___is_AEAD_BCRYPT(k1))
  {
    FStar_Dyn_dyn key;
    if (k1.tag == EverCrypt_AEAD_BCRYPT)
    {
      key = k1.case_AEAD_BCRYPT;
    }
    else
    {
      key = KRML_EABORT(FStar_Dyn_dyn, "unreachable (pattern matches are exhaustive in F*)");
    }
    EverCrypt_BCrypt_aead_encrypt(key, iv, ad, adlen, plaintext, len, cipher1, tag);
  }
  else
  {
...
)
{
  ...
  else if (true && EverCrypt_uu___is_AEAD_BCRYPT(k1))
  {
    ...
    return EverCrypt_BCrypt_aead_decrypt(key, iv, ad, adlen, plaintext, len, cipher1, tag);
  }
  else
  {
