/* 
  ...
  KreMLin invocation: C:\cygwin64\home\everest\everest\kremlin\_build\src\Kremlin.native -verbose -verbose -I C:/cygwin64/home/everest/everest/hacl-star/code/old/lib/kremlin -I C:/cygwin64/home/everest/everest/kremlin/kremlib/compat -I C:/cygwin64/home/everest/everest/hacl-star/specs -I C:/cygwin64/home/everest/everest/hacl-star/specs/old -I . -fparentheses -fcurly-braces -fno-shadow -fparentheses -falloca -ftail-calls -ccopt -march=native -verbose -ldopt -flto -bundle Crypto.Symmetric.AES+Crypto.Symmetric.AES128=*[rename=Hacl_AES] -tmpdir aesgcm-c -minimal -add-include "kremlib.h" -skip-compilation aesgcm-c/out.krml
  F* version: 946ec3ee
...
static void
Crypto_Symmetric_AES_subBytes_aux_sbox(uint8_t *state0, uint8_t *sbox0, uint32_t ctr0)
{
...
static void
Crypto_Symmetric_AES_cipher_loop(uint8_t *state0, uint8_t *w0, uint8_t *sbox0, uint32_t round0)
{
  uint8_t *state = state0;
  uint8_t *w = w0;
  uint8_t *sbox = sbox0;
  uint32_t round = round0;
  while (true)
  {
...
static uint8_t Crypto_Symmetric_AES_rcon(uint32_t i0, uint8_t tmp0)
{
  uint32_t i = i0;
  uint8_t tmp = tmp0;
  while (true)
...
static void
Crypto_Symmetric_AES_keyExpansion_aux(uint8_t *w0, uint8_t *temp0, uint8_t *sbox0, uint32_t j0)
{
  uint8_t *w = w0;
  uint8_t *temp = temp0;
  uint8_t *sbox = sbox0;
...
static void
Crypto_Symmetric_AES_invSubBytes_aux_sbox(uint8_t *state0, uint8_t *sbox0, uint32_t ctr0)
{
  uint8_t *state = state0;
  uint8_t *sbox = sbox0;
  uint32_t ctr = ctr0;
...
static void
Crypto_Symmetric_AES_inv_cipher_loop(
  uint8_t *state0,
  uint8_t *w0,
  uint8_t *sbox0,
  uint32_t round0
...
static void
Crypto_Symmetric_AES128_subBytes_aux_sbox(uint8_t *state0, uint8_t *sbox0, uint32_t ctr0)
{
  uint8_t *state = state0;
  uint8_t *sbox = sbox0;
  uint32_t ctr = ctr0;
...
static void
Crypto_Symmetric_AES128_cipher_loop(
  uint8_t *state0,
  uint8_t *w0,
  uint8_t *sbox0,
  uint32_t round0
...
static uint8_t Crypto_Symmetric_AES128_rcon(uint32_t i0, uint8_t tmp0)
{
  uint32_t i = i0;
  uint8_t tmp = tmp0;
  while (true)
...
static void
Crypto_Symmetric_AES128_keyExpansion_aux(
  uint8_t *w0,
  uint8_t *temp0,
  uint8_t *sbox0,
  uint32_t j0
...
static void
Crypto_Symmetric_AES128_invSubBytes_aux_sbox(uint8_t *state0, uint8_t *sbox0, uint32_t ctr0)
{
  uint8_t *state = state0;
  uint8_t *sbox = sbox0;
  uint32_t ctr = ctr0;
